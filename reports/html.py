...
def sizing_calc_report(time_range: TimeRange, test_details: TestDetails,
                       data: pd.DataFrame, folder: Path, file_name: str,
                       test_summary: Optional[TestSummary] = None):
    """Create and save full report to file"""
    file_name = f'{file_name}_{str(time_range)}.html'
    report_header = sizing_calc_report_header(test_details, time_range) \
