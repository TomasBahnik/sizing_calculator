...
MEMORY_RESOURCE: Resource = Resource(
    name="memory",
    unit="Mi",
    limit="MEMORY_LIMIT_BYTE",
    measured="MEMORY_BYTE",
    request="MEMORY_REQUEST_BYTE",
)

CPU_RESOURCE: Resource = Resource(
    name="cpu",
    unit="m",
    limit="CPU_LIMIT_CORE",
    measured="CPU_CORE",
    request="CPU_REQUEST_CORE",
)

JVM_PROCESS_HEAP_RESOURCE: Resource = Resource(
    name="jvm_process_heap",
    unit="Mi",
    limit="JVM_PROCESS_HEAP_MAX",
    measured="JVM_PROCESS_HEAP_USED",
)

JVM_PROCESS_NON_HEAP_RESOURCE: Resource = Resource(
    name="jvm_process_non_heap",
    unit="Mi",
...
class SizingCalculator:
    ...
    def from_test_details(
        cls, cpu: LimitsRequests, memory: LimitsRequests, test_details: TestDetails
    ) -> SizingCalculator:
        return cls(
            cpu=cpu,
            memory=memory,
...
def save_new_sizing(
    ...
    if len(all_test_sizing) > 0:
        ...
        new_sizings_report = (
            sizing_calc_summary_header(test_summary) + "<br/>" + new_sizings.to_html()
            if test_summary
            else new_sizings.to_html()
        )
...
def sizing_ini(new_sizings: pd.DataFrame, folder: Path):
    ...
    for container, row in new_sizings.iterrows():
        ...
        config.set(
            ...
            value=str(row[MEMORY_LIMIT_NAME]) + "Mi",
        )
        config.set(
            section=section,
