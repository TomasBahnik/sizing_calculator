...
from metrics import TIMESTAMP_COLUMN, POD_BASIC_RESOURCES_TABLE, NAMESPACE_COLUMN, CONTAINER_COLUMN, POD_COLUMN
...
class RatioRule:
    ...
    def validate_limits(self):
        ...
        if self.limit_value and self.limit_column:
            raise ValueError(f"{self.basic_rule.resource}: "
    ...
    def missing_idx_values(self):
        ...
        missing_values: pd.DataFrame = \
            resource_values_ts.loc[resource_values_ts.index.isin(self.missing_limits_idx()), :]
    ...
    def over_pct_static_limit(self) -> pd.DataFrame:
        ...
        if compare == Compare.GREATER:
            ...
            m_idx: pd.MultiIndex = pd.MultiIndex.from_tuples(list(self.containerPodIndexes.keys()),
                                                             names=CONTAINER_POD_COLUMNS)
            over_pct: pd.DataFrame = pd.DataFrame(data=data, index=m_idx)
            over_pct_static: pd.DataFrame = \
                over_pct[over_pct[self.basic_rule.resource] > self.basic_rule.resource_limit_value]
    ...
    def report_header(self):
        ...
        data: Dict[str, List[str]] = {'namespace': [self.get_namespace()],
    ...
    def add_report(self, main_header: str, resource_table: Optional[PortalTable] = None) -> str:
        ...
        self.report_df = pd.concat([self.report_df, requests_limits_df], axis=1, join='inner') \
            if requests_limits_df is not None else self.report_df
        if self.report_data():
...
@app.command()
def eval_rules(start_time: str = typer.Option(None, "--start", "-s",
